...
%DIF DEL submitted/orig/hertz.tex                             Fri Oct 20 09:22:52 2023
%DIF ADD submitted/rev-1/Cycle-restrictions-under-Foata.tex   Mon Oct 23 10:03:56 2023
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{amsfonts}
\usepackage{amsthm}
...
%DIF <  \begin{figure}
%DIF <  \begin{alignat*}{2}
%DIF <    r_1 &= \pattern{}{1}{1/1}{0/0,0/1,1/0}
%DIF <    & s_1 &= \pattern{}{2}{1/1,2/2}{0/1,0/2,1/1,1/2,2/1}\\[1.5ex]
%DIF <    r_2 &= \pattern{}{2}{2/1,1/2}{0/0,0/1,0/2,1/0,1/1,1/2,2/0,2/1}
%DIF <    & s_2 &= \pattern{}{3}{1/1,2/3,3/2}{0/1,0/2,0/3,1/1,1/2,1/3,2/0,2/1,2/2,2/3,3/1,3/2} \\[1.5ex]
%DIF <    r_3 &= \pattern{}{3}{3/1,2/2,1/3}{
%DIF <      0/0,0/1,0/2,0/3,1/0,1/1,1/2,1/3,2/0,2/1,2/2,2/3,3/0,3/1,3/2}\qquad
%DIF <    & s_3 &= \pattern{}{4}{1/1,2/4,3/3,4/2}{
%DIF <      0/1,0/2,0/3,0/4,1/1,1/2,1/3,1/4,2/0,2/1,2/2,2/3,2/4,3/0,3/1,3/2,3/3,3/4,4/1,4/2,4/3} \\
%DIF <  \end{alignat*}
%DIF <  \caption{The mesh patterns $r_q$ and $s_q$}\label{mesh-patterns}
%DIF <  \end{figure}
...
\begin{proof}
  ...
  \]
  \DIFdelbegin \DIFdel{It }\DIFdelend \DIFaddbegin \DIFadd{Term-wise differentiation yields
  }\[
    \DIFadd{\frac{d}{dx}\biggl(\frac{F(x)}{1+x^2}\biggr)
    = \frac{(1-x-x^2)F(x)-1+x^2}{x^2(1+x^2)}
  }\]
  \DIFadd{and using this it }\DIFaddend is easy to verify that $F(x)/(1+x^2)$ satisfies the same
  differential equation \DIFaddbegin \DIFadd{as $A(x)$}\DIFaddend . It thus suffices to show that
...
%DIF <  We highlight that the first author also stated an alternative version of the conjecture,
%DIF <  by noting that the compositional inverse of $x/(1+x^2)$ is $xC(x^2)$, where $C(x)$ is the
%DIF <  generating function for the Catalan numbers. This implies that the conjecture we have
%DIF <  proven is equivalent to
%DIF <  \MATHBLOCKSQUAREBRACKET[[x^n]]{F(xC(x^2))=n!
%DIF <  }
%DIF <  Structurally this should be viewed as a
%DIF <  saying that any permutation can be uniquely obtained from a permutation that avoids $p$, by
%DIF <  replacing each of its points with a unique permutation from a family of permutations
%DIF <  enumerated by $xC(x^2)$. We have not been able to describe this structural decomposition.
\DIFaddbegin \DIFadd{As above, assume that $\pi\mapsto\sigma$ under Foata's fundamental
...
mesh pattern
  ...
  \DIFadd{\pattern{}{1}{1/1}{0/0}
}\]
\DIFadd{in $\sigma$. We have shown that the statistic ``number of
fixed points'' translates to the mesh pattern statistic
}\[
  \DIFadd{\pattern{scale=0.95}{1}{1/1}{0/0,1/0,1/1} \;+\, 
  \pattern{}{2}{1/2,2/1}{0/0,0/1,1/0,1/1,1/2}
}\]
\DIFadd{and more generally that the number of adjacent $q$-cycles translates to
$r_q+s_q$. A fixed point $\pi(i)=i$ of $\pi$ is said to be }\emph{\DIFadd{strong}}
\DIFadd{if $j<i\Rightarrow \pi(j)<\pi(i)$ and $j>i\Rightarrow \pi(j)>\pi(i)$;
see \mbox{%DIFAUXCMD
\cite[Ex.\ 1.32b]{EC1}}\hskip0pt%DIFAUXCMD
. In term of mesh patterns, strong fixed
...
}\emph{\DIFadd{skew strong fixed points}}\DIFadd{~\mbox{%DIFAUXCMD
\cite{Bra11}}\hskip0pt%DIFAUXCMD
...
%DIFDELCMD < \bibliography{hertz}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{thebibliography}{1}

\bibitem{Bra11}
\DIFadd{Petter Br}{\DIFadd{\"a}}\DIFadd{nd}{\DIFadd{\'e}}\DIFadd{n and Anders Claesson.
}\newblock \DIFadd{Mesh patterns and the expansion of permutation statistics as sums of
  permutation patterns.
}\newblock {\em \DIFadd{Electron. J. Combin}}\DIFadd{, 18(2):P5, 2011.
}

\bibitem{Brualdi2012}
\DIFadd{Richard~A. Brualdi and Emeric Deutsch.
}\newblock \DIFadd{Adjacent }{\DIFadd{$q$}}\DIFadd{-cycles in permutations.
}\newblock {\em \DIFadd{Ann. Comb.}}\DIFadd{, 16(2):203--213, 2012.
}

\bibitem{Cl2022}
\DIFadd{Anders Claesson.
}\newblock \DIFadd{From }{\DIFadd{H}}\DIFadd{ertzsprung's problem to pattern-rewriting systems.
}\newblock {\em \DIFadd{Algebraic Combinatorics}}\DIFadd{, 5(6):1257--1277, 2022.
}

\bibitem{foata}
\DIFadd{Dominique Foata.
}\newblock \DIFadd{\'{E}tude alg\'ebrique de certains probl\'emes d'analyse combinatoire
  et du calcul des probabilit\'es.
}\newblock {\em \DIFadd{Publ. Inst. Statist. Univ. Paris}}\DIFadd{, 14:81--241, 1965.
}

\bibitem{shading}
\DIFadd{\'I. Hilmarsson, I.~J\'onsd\'ottir, S.~Sigurdard\'ottir, L.~Vidarsd\'ottir, and
  Henning Ulfarsson.
}\newblock \DIFadd{Wilf-classification of mesh patterns of short length.
}\newblock {\em \DIFadd{Electronic Journal of Combinatorics}}\DIFadd{, 22, 2015.
}

\bibitem{OEIS}
\DIFadd{OEIS~Foundation Inc.
}\newblock \DIFadd{The }{\DIFadd{O}}\DIFadd{nline }{\DIFadd{E}}\DIFadd{ncyclopedia of }{\DIFadd{I}}\DIFadd{nteger }{\DIFadd{S}}\DIFadd{equences.
}\newblock \texttt{\DIFadd{https://oeis.org/}}\DIFadd{, 2023.
}

\bibitem{EC1}
\DIFadd{Richard~P. Stanley.
}\newblock {\em \DIFadd{Enumerative combinatorics}}\DIFadd{, volume~1 of }{\em \DIFadd{Cambridge Studies
  in Advanced Mathematics}}\DIFadd{.
}\newblock \DIFadd{Cambridge University Press, 2 edition, 2011.
}
